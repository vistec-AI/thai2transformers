...
Script adapted from https://github.com/n-waves/multifit
"""
import argparse
...
from pythainlp.ulmfit import rm_useless_spaces, rm_useless_newlines
    ...
    with open(file_path, mode, encoding='utf-8') as f_out:
        for i, text in enumerate(text_iter):
            ...
            for paragraph in paragraphs:
                paragraph = mt.clean_wiki(paragraph)
                tokenized = '|'.join(mt.tokenize(paragraph.strip()))
                tokenized_paragraphs.append(tokenized.replace('|',''))
...
def main(args):
    ...
    mt = ThaiTokenizer(word_tokenize)
